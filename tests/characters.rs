...
fn fixture(root: &Path) -> std::path::PathBuf {
    ...
    for (id, folder, colors) in [
        ...
        ("hemlock", "Hemlock", vec!["#28323C"]),
        ("louis", "Louis", vec!["#28323C"]),
        ("luc", "Luc", vec!["#28323C"]),
        ("maple", "Maple", vec!["#28323C"]),
        ("merri", "Merri", vec!["#28323C"]),
        ...
        ("caldarus", "Caldarus", vec!["#28323C"]),
        ("seridia", "Seridia", vec!["#28323C"]),
    ] {
...
fn characters_can_be_built_alone_or_with_existing_characters() {
    for (ids, label, hotkey) in [
        ...
        (vec!["wiscar"], "Wiscar", "W"),
        (vec!["wynne"], "Wynne", "Q"),
        (vec!["caldarus"], "Caldarus", "DELETE"),
        (vec!["seridia"], "Seridia", "S"),
        (
            vec![
                "adeline",
                "hayden",
