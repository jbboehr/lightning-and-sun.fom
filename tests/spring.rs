...
#[test]
fn export_supports_a_complete_25_portrait_selection() {
    ...
    assert_eq!(report["files"].as_array().unwrap().len(), 25);
    for i in 0..25 {
        assert_eq!(
...
#[test]
fn export_rejects_empty_duplicate_and_26_asset_selections_before_writing() {
    let temp = tempfile::tempdir().unwrap();
    let source = temp.path().join("source");
